...
class AddNotificationTableViewController: UITableViewController, THContactPickerViewControllerDelegate {
    ...
    func didFinishSelectingContacts(contactPhoneNumbers: [AnyObject]!) {
        ...
        for contact in contactPhoneNumbers{
            if(contact.firstName != nil && contact.phone != nil){
                print("\"" + contact.phone + "\",")
                if(contact.lastName != nil && contact.lastName != "nil" ){
                    self.names.append("\(contact.firstName) \(contact.lastName)")
                }else{
                    self.names.append("\(contact.firstName)")
                }
                self.phoneNumbers.append(contact.phone)
            }
        }
