...
@property (nonatomic, strong) NSArray * initiallySelectedPhoneNumbers;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil initiallySelectedPhoneNumbers:(NSArray*)phoneNumbers
{
    ...
    if (self) {
        ...
        self.initiallySelectedPhoneNumbers = phoneNumbers;
    }
    return self;
}
...
-(void)getContactsFromAddressBook
{
    ...
    if (addressBook) {
        ...
        for(int i = 0 ; i < self.contacts.count; i++){
            THContact * currentContact = ((THContact*)[self.contacts objectAtIndex:i]);
            if([self.initiallySelectedPhoneNumbers containsObject:currentContact.phone]){
                [self.selectedContacts addObject:currentContact];
                [self.contactPickerView addContact:currentContact withName:currentContact.fullName];
            }
        }
...
- (void)done:(id)sender
{
    if ([self.delegate respondsToSelector:@selector(didFinishSelectingContacts:)]) {
        [self.delegate didFinishSelectingContacts:self.selectedContacts];
    }
}
